...
class MOSIFactory(loading.BaseDMVRFactory, abc.ABC):
  """Base class for a toy factory."""

  _BASE_DIR = "..\\..\\..\\data_samples\\"
  _NUM_CLASSES = 7
  ...
  def __init__(self, subset = 'train', split = 1):
    ...
    del split
    if subset not in MOSIFactory._TABLES:
      raise ValueError('Invalid subset "{}". The available subsets are: {}'
                       .format(subset, MOSIFactory._TABLES.keys()))

    super().__init__(MOSIFactory._BASE_DIR,
                     table=f'{subset}',
                     source='tfrecord',
  ...
  def _build(self,
    ...
    processing.add_audio(
        ...
        preprocessor_builder=self.preprocessor_builder,
        postprocessor_builder=self.postprocessor_builder,
        input_feature_name= "WAVEFORM/feature/floats", #'waveform/floats', # change to the original dmvr setting
    ...
    processing.add_label(
        ...
        decoder_builder=self.decoder_builder,
        preprocessor_builder=self.preprocessor_builder,
        input_label_index_feature_name= "clip/label/index", #'label/index', # change to the original dmvr setting
        input_label_name_feature_name= "clip/label/text", #'label/text', # change to the original dmvr setting
